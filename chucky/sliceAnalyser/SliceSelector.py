...
class SliceSelector(object):
    ...
    def select_all(self):#return list of (statement,symbol)
        if self.job.symbol_type == 'CallExpression':
            ...
            return self._select_by_identifier_decl()
        else:
            return []
    ...
    def _select_by_identifier_decl(self):
        node_selection = "queryNodeIndex('type:IdentifierDecl')"
        traversal_steps = []
        traversal_steps.append("as('identifier')")
        traversal_steps.append("children().filter{{it.type == 'Identifier' && it.code == '{}'}}.back('identifier')".format(self.job.symbol_name))
        traversal_steps.append("children().filter{{it.type == 'IdentifierDeclType' && it.code == '{}'}}.back('identifier')".format(self.job.symbol_decl_type))
        traversal = '.'.join(traversal_steps)
        transform = "statements().transform{ [it.id, it, it.defines().next().id, it.defines().next()] }"
        command = '.'.join([node_selection, traversal, transform])
        results = jutils.runGremlinCommands([command])
        if len(results)>0:
            return map(lambda x : (Statement(x[0], x[1].get_properties()), Symbol(x[2], x[3].get_properties())), results)
